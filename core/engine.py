...
_MD_HEAD_RE = re.compile(r"(?<![\n#])(#{2,4} )")


def _md_fix_text(raw: str, drop_preamble: bool = True) -> str:
    """마크다운 보정: 첫 '## ' 헤더 이전의 진행 멘트 제거 + 줄 중간에 붙은 헤더 앞에 빈 줄 삽입."""
    txt = raw
    if drop_preamble:
        i = txt.find("## ")
        if i == -1:
            return ""
        txt = txt[i:]
    return _MD_HEAD_RE.sub(r"\n\n\1", txt).lstrip("\n")


def _md_stream_fix(gen, drop_preamble: bool = True):
    """스트리밍 중에도 마크다운이 항상 올바르게 렌더되도록 보정하는 래퍼.
    청크 경계에서 '##'가 쪼개져도 안전하게 뒤 8자를 보류했다가 내보낸다."""
    raw, sent = "", 0
    for t in gen:
        raw += t
        fixed = _md_fix_text(raw, drop_preamble)
...
def research_company_stream(client, model: str, company: str, role: str,
    ...
    return _md_stream_fix(_stream_with_search(
        client, model, prompts.RESEARCH_SYSTEM, user,
...
def analyze_pass_essays_stream(client, model: str, company: str, role: str,
    ...
    return _md_stream_fix(_stream_with_search(
        client, model, prompts.PASS_SYSTEM, user,
