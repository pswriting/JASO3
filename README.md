...
| ⛏️ 소재 발굴 | 이력서 파일과 스펙에서 AI가 문항 유형별 소재를 초안까지 발굴. 막막하면 '기억 자극 질문' 도우미 제공 |
| 📎 파일 자동 추출 | 이력서·경력기술서·기존 자소서(PDF·DOCX·TXT·HWP·HWPX)를 올리면 내용을 자동 추출해 반영 |
| 🧭 단계형 진행 | ①→⑤ 단계 버튼과 하단 이전/다음 버튼으로 순서대로 진행 |
...
2. **DART 전자공시 연동 — 기본 탑재** (별도 키 불필요)
   - 무료 인증키가 내장돼 있어 상장사 재무제표·공시가 자동 분석됩니다.
   - 내 키로 바꾸려면: <https://opendart.fss.or.kr>에서 발급 후 Streamlit **Secrets**에 `DART_API_KEY = "키"` 추가.
...
3. **③ 소재 발굴** — 이력서·기존 자소서 파일 업로드(자동 추출) → AI가 소재를 초안까지 발굴. 막막하면 '기억 자극 질문' 활용
4. **④ 자소서 생성** — 문항 붙여넣기 → **발굴 소재 불러오기 또는 경험 입력(여기 한 곳에서만)** → 분량(300~5000자) 선택 → 생성 → AI 감지·유사도 검사
